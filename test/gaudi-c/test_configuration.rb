...
class TestSystemConfiguration < Minitest::Test
  ...
  def test_external_libraries
    ...
    File.expects(:exist?).with(File.expand_path(File.join(__dir__, "foo.lib"))).returns(false)
...
class TestBuildConfiguration < Minitest::Test
  ...
  def test_basic_configuration
    ...
    system_cfg.stubs(:config_base).returns(__dir__)
    ...
    assert_equal(["#{File.expand_path(__dir__)}/inc"], cfg.incs)
    ...
    File.expects(:exist?).with(File.expand_path(File.join(__dir__, "foo.lib"))).returns(false)
    File.expects(:exist?).with(File.expand_path(File.join(__dir__, "bar.lib"))).returns(false)
...
class TestPlatformConfiguration < Minitest::Test
  ...
  def test_platform_extend
    system_config = Gaudi::Configuration::SystemConfiguration.new(File.join(__dir__, "tmp/brain.cfg"))
    component = mock
